...
@export_custom(PROPERTY_HINT_LINK, "") var follow_damping_value: Vector2 = Vector2(0.1, 0.1):
...
func _validate_property(property: Dictionary) -> void:
	...
	if property.name == "follow_target":
		if follow_mode == FollowMode.NONE or follow_mode == FollowMode.GROUP:
			property.usage = PROPERTY_USAGE_NO_EDITOR

	elif property.name == "follow_path" and follow_mode != FollowMode.PATH:
		property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if follow_mode == FollowMode.NONE:
		match property.name:
			"follow_offset", "follow_damping", "follow_damping_value", "follow_axis_lock", "rotate_with_target":
				property.usage = PROPERTY_USAGE_NO_EDITOR

	if property.name == "follow_offset":
		if follow_mode == FollowMode.PATH or follow_mode == FollowMode.GLUED:
			property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if follow_mode != FollowMode.GROUP:
		match property.name:
			"follow_targets", "auto_zoom":
				property.usage = PROPERTY_USAGE_NO_EDITOR

	if not auto_zoom or follow_mode != FollowMode.GROUP:
		match property.name:
			"auto_zoom_min", "auto_zoom_max", "auto_zoom_margin":
				property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if not follow_mode == FollowMode.FRAMED:
		match property.name:
			"dead_zone_width", "dead_zone_height", "show_viewfinder_in_play":
				property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if not rotate_with_target or follow_mode == FollowMode.GROUP:
		match property.name:
			"rotation_damping", "rotation_offset", "rotation_damping_value":
				property.usage = PROPERTY_USAGE_NO_EDITOR
...
func _process(delta: float) -> void:
	if _follow_target_physics_based or _is_active:
		return
	process_logic(delta)


func _physics_process(delta: float) -> void:
	if not _follow_target_physics_based or _is_active:
		return
	process_logic(delta)
	...
	match follow_mode:
		...
		FollowMode.GROUP:
			if _has_multiple_follow_targets:
				...
				if auto_zoom:
					rect = rect.grow_individual(auto_zoom_margin.x, auto_zoom_margin.y, auto_zoom_margin.z, auto_zoom_margin.w)
		...
		FollowMode.PATH:
			...
			_follow_target_position = follow_path.curve.get_closest_point(_get_target_position_offset() - path_position) + path_position
...
func _interpolate_position(target_position: Vector2, delta: float) -> void:
	...
	if rotate_with_target:
		if rotation_damping and not Engine.is_editor_hint():
			output_rotation = _smooth_damp(follow_target.get_rotation() + rotation_offset, _transform_output.get_rotation(), 0, _rotation_velocity_ref, _set_rotation_velocity, rotation_damping_value, delta)
		else:
	...
	if follow_damping and not Engine.is_editor_hint():
		...
		for i in 2:
			output_position[i] = _smooth_damp(global_position[i], _transform_output.origin[i], i, _follow_velocity_ref[i], _set_follow_velocity, follow_damping_value[i], delta)
...
func _is_parents_physics(target: Node = self) -> void:
	...
	while current_node:
		...
		if not current_node is PhysicsBody2D:
			continue
		_follow_target_physics_based = true
...
func update_limit_all_sides() -> void:
	...
	if not is_instance_valid(_limit_node):
		...
		if not tile_map.tile_set:
			return  # TODO: This should be removed once https://github.com/godotengine/godot/issues/96898 is resolved
		...
		limit_rect = Rect2(tile_map_position + Vector2(limit_margin.x, limit_margin.y), tile_map_size - Vector2(limit_margin.x, limit_margin.y) - Vector2(limit_margin.z, limit_margin.w))
		...
		if not collision_shape_2d.get_shape():
			return
		...
		## Calculates the Rect2 based on the Tile Map position and size + margin
		limit_rect = Rect2(limit_rect.position + Vector2(limit_margin.x, limit_margin.y), limit_rect.size - Vector2(limit_margin.x, limit_margin.y) - Vector2(limit_margin.z, limit_margin.w))
