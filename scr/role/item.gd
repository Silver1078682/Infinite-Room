...
const Drop := preload("res://scr/role/items/drop/drop.gd")
...
static func drop_at(coord: Vector2i, item: Item, intial_vectoity := _get_random_velocity()) -> Drop:
	return drop_at_global_pos(Main.Map.to_pos(coord), item, intial_vectoity)


## drop an item at the global position, also returns the drop
static func drop_at_global_pos(position: Vector2, item: Item, intial_vectoity := _get_random_velocity()) -> Drop:
	var drop: Drop = Drop.SCENE.instantiate()
	drop.global_position = position
	drop.item = item
	drop.linear_velocity = intial_vectoity
...
func _add_block():
	meta["block"] = name
	if FileAccess.file_exists(Block.CONFIG_DIR_PATH % name):
