...
## Main.Map
## use built-in [TileMapLayer] to render textures and calculate terrain collision.
...
## change a human-readable coordinate (y axis pointing up)  to a normal coordinate (y axis pointing down)
static func h2gui(coord: Vector2i) -> Vector2i:
	return Vector2i(coord.x, Room.HEIGHT - coord.y - 1)

## automatically connect the tiles
